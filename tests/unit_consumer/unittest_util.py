...
_FIXTURE_PATH = os.path.join(_TESTS_PATH, '../fixtures_consumer')
_FIXTURE_FINAL_PATH = os.path.join(_TESTS_PATH, '../fixtures_consumer_final')


def gen_consumer_api(check):
...
def gen_consumer_final_api(check):
    @urlmatch(netloc=r'(.*\.)?endpoint')
    def datahub_api_mock(url, request):
        check(request)
        path = url.path.replace('/', '.')[1:]
        if request.body:
            ...
            path += ('.' + content['Action'])
        res_file = os.path.join(_FIXTURE_FINAL_PATH, '%s.json' % path)
        status_code = 200
        content = {
        }
        headers = {
